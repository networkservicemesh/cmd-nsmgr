...
func NewServer(inner nsmgr.Nsmgr, closeAll *func(ctx context.Context, u string)) nsmgr.Nsmgr {
...
func (p *peerTrackerServer) Request(ctx context.Context, request *networkservice.NetworkServiceRequest) (*networkservice.Connection, error) {
	u, err := authz.CertificateURL(ctx)
	if err != nil {
		return nil, err
	}
	id := u.String()
	...
	p.executor.AsyncExec(func() {
		...
		if !ok {
			p.connections[id] = make(map[string]*networkservice.Connection)
		}
		p.connections[id][conn.GetId()] = conn
	})
	return conn, nil
}
...
func (p *peerTrackerServer) Close(ctx context.Context, conn *networkservice.Connection) (*empty.Empty, error) {
	...
	id := u.String()
	p.executor.AsyncExec(func() {
		delete(p.connections[id], conn.GetId())
	})
	return &empty.Empty{}, nil
}

func (p *peerTrackerServer) closeAllConnectionsForPeer(ctx context.Context, u string) {
	finishedChan := make(chan struct{})
	<-p.executor.AsyncExec(func() {
		if connMap, ok := p.connections[u]; ok {
